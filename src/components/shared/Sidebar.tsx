...
export const Sidebar: React.FC<SidebarProps> = ({ variant }) => {
  ...
  return (
    <div className="w-64 bg-white border-r border-gray-200 h-screen fixed left-0 top-0 flex flex-col shadow-sm transition-all duration-300">
      <div className="p-6 border-b border-gray-200 bg-white">
        <h1 className="text-2xl font-bold text-gray-900 truncate">{BUSINESS_NAME}</h1>
        <p className="text-gray-500 text-xs mt-1 font-medium">{variant === 'server' ? 'Admin Dashboard' : 'Point of Sale'}</p>
      </div>
      ...
      <div className="p-4 border-t border-gray-200">
        <button
          ...
          className="w-full flex items-center gap-3 px-4 py-3 rounded-lg bg-red-600 hover:bg-red-700 text-white transition-all duration-200 font-semibold shadow-md hover:shadow-lg"
        >
          <LogOut size={20} />
          <span>Logout</span>
        </button>
